...
public class Renderer3D
{
    ...
    /// <summary>
    /// Resets the rendering settings (alpha = 1f, all lighting disabled, fog disabled).
    /// </summary>
    ...
    public void DrawModel3D(Model3D model, Camera3D camera)
    {
        ...
        for (int i = 0; i < model.BaseModel.Meshes.Count; i++)
        {
            ...
            foreach (BasicEffect effect in mesh.Effects.Cast<BasicEffect>())
            {
                ...
                {
                    //disabling the specular color of the directional lights (they may have been set by EnableDefaultLighting())
                    //specularColor is the only directionalLight attribute not directly overriden by the renderer's own directionalLightGroup fields
                    ...
                    //handling directional effect lighting
...
public struct DirectionalLightPropertyGroup
{
    ...
    public DirectionalLightPropertyGroup(bool enabled, Vector3 direction, Vector3 diffuseColor)
    {
